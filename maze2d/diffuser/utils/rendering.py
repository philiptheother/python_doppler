...
_extent = {
    'maze2d-umaze-v1': (0, 1, 1, 0),
    'maze2d-medium-v1': (0, 1, 1, 0),
    'maze2d-large-v1': (0, 1, 0.75, 0),
}

MAZE_BOUNDS = {
    'maze2d-umaze-v1': (0, 5, 0, 5),
    'maze2d-medium-v1': (0, 8, 0, 8),
    'maze2d-large-v1': (0, 12, 0, 12)
}

BIAS_FOR_PLOT = {
    'maze2d-umaze-v1': 0.25,
    'maze2d-medium-v1': 0.25,
    'maze2d-large-v1': 0,
}

def patch_block(x, y, env_name, r=1):
    scale = MAZE_BOUNDS[env_name][-1]
    bias = BIAS_FOR_PLOT[env_name]
    x = (x+bias) / scale
    y = (y+bias) / scale
    return patches.Rectangle(xy = (x, y),
                             width = r/scale,
                             height = r/scale,
                             edgecolor = 'black',
                             facecolor = 'gainsboro',
...
class MazeRenderer:
    ...
    def renders(self, observations, conditions=None, title=None):
        ...
        if 'maze2d-umaze-v1' == self.env_name:
            centers_umaze_v1 = [
                (0.55,3.05),
                (1.05,3.05),
                (1.55,3.05),
                ...
                (2.55,3.05),
                (3.05,3.05),]
            for idx, center in enumerate(centers_umaze_v1):
                ax.add_patch(patch_block(center[1],center[0],self.env_name,r=0.5))
        elif 'maze2d-medium-v1' == self.env_name:
            centers_medium_v1 = [
                (1.95,1.15),
                (2.65,3.15),
                (1.95,4.45),
                ...
                (4.15,3.65),
                (3.65,4.45),]
            for idx, center in enumerate(centers_medium_v1):
                ax.add_patch(patch_block(center[1],center[0],self.env_name,r=0.5))
        elif 'maze2d-large-v1' == self.env_name:
            centers_large_v1 = [
                (3,2),
                (2,4),
                (2,6),
                ...
                (5,7),
                (5,9),]
            for idx, center in enumerate(centers_large_v1):
                ax.add_patch(patch_block(center[1],center[0],self.env_name,r=1.0))
...
class Maze2dRenderer(MazeRenderer):
    ...
    def __init__(self, env, observation_dim=None, output=print):
        ...
        self._remove_margins = False
        self._extent = _extent[self.env_name]
        self.output = output
